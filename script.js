...
if(stored_list==null){
  ...
  len=stored_list.length;
    for(i=0;i<len;i++){
        ...
        var str=`<tr><th scope="row">`+(i+1).toString()+`</th><td>`+temp_dict['username']+`</td><td>`+temp_dict['category']+`</td><td><input id="`+(i+1).toString()+`" class="del" type="image" src="trash-icon.jpg" width="35" height="25" alt="X"
        /></td></tr>`;
